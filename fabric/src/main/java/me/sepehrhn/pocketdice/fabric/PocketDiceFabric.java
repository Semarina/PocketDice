        ...
        ServerPlayConnectionEvents.JOIN.register((handler, sender, server) -> {
            if (FabricConfig.DATA.updates.enabled && FabricConfig.DATA.updates.notify_admins_on_join) {
                ...
                if (result != null && result.getStatus() == UpdateCheckStatus.UPDATE_AVAILABLE) {
                    ...
                    boolean isOp = java.util.stream.Stream.of(server.getPlayerManager().getOpNames()).anyMatch(name -> name.equalsIgnoreCase(playerName));
                    if (isOp) {
                        ...
                        net.minecraft.text.Text message = me.sepehrhn.pocketdice.fabric.locale.FabricLocaleManager.getText(server.getRegistryManager(), localeCode, "messages.update.available_admin", placeholders);
