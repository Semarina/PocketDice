...
public final class DiceParser {
    ...
    public static Component formatResult(String playerName, DiceRoll result) {
        String format = "<gray>[PocketDice] <yellow><player></yellow> rolled <aqua><dice>d<faces></aqua>: <gold><rolls></gold> <gray>(Total: <green><total></green>)</gray>";
        
        return MiniMessage.miniMessage().deserialize(format,
            Placeholder.unparsed("player", playerName),
            Placeholder.unparsed("dice", String.valueOf(result.spec().dice())),
            Placeholder.unparsed("faces", String.valueOf(result.spec().faces())),
            Placeholder.parsed("rolls", result.rolls().toString()),
            Placeholder.unparsed("total", String.valueOf(result.total()))
        );
    }
